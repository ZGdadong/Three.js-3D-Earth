...
- **地球上的中国**：`buildChinaRegionOnEarth()` 用全国 GeoJSON 把中国**轮廓**贴合到地球球面上，画成一条**金色静态轮廓线**（`chinaLineMat`，颜色/透明度在「🌏 中国轮廓」面板可调：`earthChinaLineColor/Opacity`）。另用一个**不可见拾取网格**（`chinaHitMat`，`colorWrite:false`）覆盖中国区域用于悬停/点击检测。**悬停中国**高亮（线变亮）并浮出"中国"提示；**点击中国**触发 `chinaMap.playCloudWipe()` 的**云带右→左过渡**，云盖住屏幕时回调 `setMode('china')` 切到中国地图，云再淡出露出省份图（`updateCloudWipe` 由 `main.js` 的 `animate` 每帧推进，不依赖模式）。
