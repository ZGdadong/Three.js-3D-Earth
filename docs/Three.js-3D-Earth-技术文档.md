...
- **地球上的中国**：`buildChinaRegionOnEarth()` 用全国 GeoJSON 把中国**轮廓**贴合到地球球面上：一条**金色静态轮廓线**（`Line2` 粗线，清晰可见）+ 一个**沿轮廓流动的亮头彗星**（`THREE.Points` + `aIndex/uTime` 着色器，把每条边**加密采样**（`COMET_SAMPLES`）让拖尾连续；片元按 `vHead` 做**拖尾渐隐**：头部用 `uHeadColor` 亮色、尾部渐淡）。只对满足 `earthChinaMinSpan`（轮廓最小尺寸，单位度）的环画线/拖尾，**过滤掉南沙等微型岛礁**（否则几百个小岛用粗线会像蜘蛛网）。「🌏 中国轮廓」面板可调：**线颜色 / 线透明度 / 线头颜色 / 线头速度 / 线头大小 / 线头透明度 / 线尾长度(拖尾) / 静态线闪烁速度 / 静态线闪烁强度 / 轮廓最小尺寸(度)**（`earthChinaLineColor/Opacity/HeadColor/CometSpeed/CometSize/CometOpacity/CometLength/BlinkSpeed/BlinkAmount/MinSpan`）。另用一个**不可见拾取网格**（`chinaHitMat`，`colorWrite:false`）覆盖中国区域用于悬停/点击检测。**悬停中国**高亮（线变亮）并浮出"中国"提示；**点击中国**触发 `chinaMap.playCloudWipe()` 的**云带右→左过渡**，云盖住屏幕时回调 `setMode('china')` 切到中国地图，云再淡出露出省份图（`updateCloudWipe` 由 `main.js` 的 `animate` 每帧推进，不依赖模式）。
- 进入中国地图时用 JS 隐藏地球 lil-gui、显示中国地图的 `.china-gui` 面板（文件夹：显示 / 旋转圆环 / 扫描能量波 / 底部栅格 / 💾 参数），离开再切回。
- 中国地图顶部 header 除「← 返回」（上级/全国）外，新增 **「🌍 返回地球」** 按钮：点击触发 `chinaMap.playCloudWipe(() => setMode('earth'))`，云过渡切回地球（`chinaMap.setReturnEarthHandler` 由 `main.js` 注入）。
